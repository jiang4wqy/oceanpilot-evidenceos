...
> **历史交付记录（2026-08-16），不代表当前运行口径。** 原文保留当日输入、截图说明和测试结果以便追溯；其中“全部 Synthetic 材料内容一致”等旧审核示例不能沿用：系统只登记材料元数据，未读取或核验真实正文。旧版 mock 发送终点、清空数据库排练及当时测试数量也不作为当前验收证明。2026-09-06 起主演示以双端同案协作、A/B/C 新建副本、当前版本人工登记复核与摘要导出为准，最终发送由后端关闭。当前步骤见 [双端演示 Runbook](demo.md)，接口见 [工作台 API 合同](implementation/2026-09-06-workspace-contract.md)。
