...
| ⑦ | Prevention | 基于 synthetic 信号事前评估拒付倾向、建议留证；真实信号待公司输入 | SignalSource | 中 |
| ⑧ | 护栏（横切） | PII 脱敏、模型分级路由、禁资金动作、审计、人工闸门 | Redactor, PolicyGate, Audit | — |

**编排要点**：确定性闸门 + 人在环；③的规则判定与胜诉结构走确定性引擎，只有「解释文字/文书起草」走 LLM；⑤只有人工批准才调用 mock connector，⑥当前只计算时限结果，不改变生产状态。会议提出的「80% agent + 高风险人工」是待真实流程/数据验证的目标，不是 `v0.2.1` 的业务效果结论。
