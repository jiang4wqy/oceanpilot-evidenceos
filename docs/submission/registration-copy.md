...
- 字符数：552（保留四个段间空行）；544（移除全部换行）

## 当前事实边界

- 当前已验证：建案、证据存储、完整度判断、revision、SQLite 原子事务与审计边界。
- 当前诊断 API 固定返回 `HTTP 501 FEATURE_DEFERRED`。
- 飞书 Agent、诊断编排、真实 Oceanpayment 只读适配、Workflow、工单与 SLA 均为入围后规划能力。
