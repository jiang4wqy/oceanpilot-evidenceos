...
| 合成交易 registry | V2 争议 SQLite 的 `v21_synthetic_transactions` | IT 管理页、标准化接收服务 | IT 管理员登记合成事实，并可接收事件及处理业务；仍遵守独立终审 |
...
`/api/v2/governance` 只向管理员和风控经理返回 `data_sources`，并在治理页展示：
...
2. IT 管理员登记本次演练的合成交易；运营人员从标准化事件入口接收它。
