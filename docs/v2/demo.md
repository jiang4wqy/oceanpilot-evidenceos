...
3. **风控专员**：在 OP 页切换风险审核角色，检查同案本版材料、来源和清单，选择 PASS 并填写审核理由。材料缺失时不能通过。
4. **OP 运营**：生成包，查看草稿、证据索引与规则来源。
5. **风控经理**：检查包内容、版本和 PII 范围，明确确认完成脱敏检查并批准冻结包。Risk 审核人和最终审批人必须为不同身份。
...
8. **风控经理**：核对状态 RECONCILED，预期净影响 12800，填写人工核对理由与合成账本引用。此时仍需通知。
9. **OP 运营**：将结果和资金摘要发布到 Portal。
10. **风控经理**：确认结案。审计展示所有角色、动作和版本。结案后的业务数据不能继续改写。
...
新建 B，切换商户查看 Visa 13.1 缺少物流和签收证明。点击提交材料被门槛阻止；Agent 显示缺项、来源、原因与下一步。可在 Portal 协作线程留言，再补齐合成材料，送 风控专员 审核；审核可退回补证、建议接受或通过。建议接受不会代替商户授权。
...
尝试结案应被阻挡。检查合成账本 CREDIT 12800 后，由 风控经理 重新核对预期 12800 并提供理由；OP 发布结果，风控经理 才能关闭。核对操作保存判断，不篡改原账本。

另可在 A 已提交后登记 OTHER + 非终局 + REPRESENTMENT，查看原阶段快照和新阶段期限。PRE_ARBITRATION 等无 fixture 阶段会进入 NEEDS_CONFIRMATION；风控专员 必须填写明确来源、版本、允许动作、证据项和期限，才能继续发布任务。
