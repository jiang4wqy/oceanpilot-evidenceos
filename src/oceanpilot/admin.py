...
from oceanpilot.web_i18n import ADMIN_I18N_SCRIPT
...
.language-control{display:flex;align-items:center;gap:7px;color:var(--muted);font-size:11.5px;white-space:nowrap}.language-control select{width:auto;min-width:92px;border:1px solid var(--border2);border-radius:8px;background:#fff;padding:7px 28px 7px 9px;color:var(--ink)}
...
.bd{min-width:0;padding:16px 18px}.badge{display:inline-flex;align-items:center;justify-content:center;padding:3px 8px;border-radius:5px;font-size:11px;font-weight:700;line-height:1.25;text-align:center;white-space:nowrap}.healthy{color:var(--good);background:var(--goodBg)}.info{color:var(--info);background:#e8f0f6}
...
.table td{padding:12px 14px;border-bottom:1px solid var(--border);font-size:12.5px}.table tr:last-child td{border-bottom:0}.table .badge{min-width:96px;max-width:144px;white-space:normal;overflow-wrap:normal;word-break:keep-all;font-size:11px;padding:5px 8px}.route{font-family:var(--mono);font-size:11.5px;color:var(--body)}
...
@media(max-width:760px){.app{grid-template-columns:1fr}.side{display:none}.top{padding:0 16px}.source{display:none}.language-control span{display:none}.content{padding:20px 14px 32px}.cards,.business{grid-template-columns:1fr}.pagehead{align-items:flex-start}.updated{display:none}}
</style>
</head>
<body>
<div class="app">
...
<main class="main"><header class="top"><div class="crumb"><b>运行维护</b> / <span id="crumb">运行总览</span></div><div class="grow"></div><label class="language-control"><span>语言</span><select id="languageSelect" aria-label="语言"><option value="zh">中文</option><option value="en">English</option></select></label><span class="env">Sandbox</span><div class="source">监控客户服务 <span id="sourceHost"></span></div><button class="refresh" id="refresh" onclick="loadOverview()">立即刷新</button></header>
...
def create_admin_app(client_base_url: str | None = None) -> FastAPI:
    ...
    page = (
        _ADMIN_HTML.replace("__CLIENT_BASE__", json.dumps(base_url.rstrip("/")))
        .replace("__OCEANPAYMENT_LOGO__", OCEANPAYMENT_LOGO_DATA_URI)
