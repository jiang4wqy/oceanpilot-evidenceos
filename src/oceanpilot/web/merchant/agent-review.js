function fillAgentPrompt(text){$('agentMessage').value=tr(text);$('agentMessage').focus();}
...
function renderAgentHistory(){
  $('agentHistory').innerHTML=S.agentMessages.slice(-12).map(item=>{
    if(item.role==='user')return `<div class="chat-bubble user" data-no-i18n>${esc(item.text)}</div>`;
    ...
    return `<div class="chat-bubble assistant"><div class="helper"><span class="pill ${source.className}">${esc(source.label)}</span>${details?`<div>${esc(details)}</div>`:''}</div><div data-no-i18n>${esc(item.text)}</div></div>`;
...
async function runAgentTurn(message,trigger='USER_MESSAGE',showUser=false){
  ...
  const context=caseContext.capture();S.failedAgentTurn=null;S.agentSubmitting=true;
  ...
  try{
    ...
    if(!result.ok){S.failedAgentTurn={caseId,revision:context.revision,message,trigger};status.className='copilot-status error';status.innerHTML='本轮说明未完成，可重试分析；本操作不批准或提交案件。<button class="tbtn" onclick="retryFailedAgentTurn()">重试分析</button>';return false;}
...
async function analyzeCurrentCase(){await runAgentTurn(S.loc==='en'?'Explain registration gaps, blockers, next step, and who must confirm for this case revision. State that file bodies have not been read.':'请基于当前案件版本，解释材料登记缺口、阻断原因、下一步和需要谁确认；明确尚未读取文件正文。','USER_MESSAGE',false);}

async function retryFailedAgentTurn(){
  const failed=S.failedAgentTurn;
  if(!failed||failed.caseId!==S.caseId)return;
  if(failed.revision!==S.caseRevision){$('agentTurnStatus').textContent='服务端案件版本已变化，请刷新本案后重新分析。';return;}
  await runAgentTurn(failed.message,failed.trigger,false);
}
