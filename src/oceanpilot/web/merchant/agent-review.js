...
function renderAgentHistory(){
  $('agentHistory').innerHTML=S.agentMessages.slice(-12).map(item=>`<div class="chat-bubble ${item.role==='user'?'user':'assistant'}">${esc(item.text)}</div>`).join('')||'<p class="empty">本次尚无对话</p>';
}
...
function renderAgentTurn(data,append=true){
  ...
  if(append&&data.assistant_message)appendAgentMessage('assistant',data.assistant_message);
...
async function runAgentTurn(message,trigger='USER_MESSAGE',showUser=false){
  ...
  try{
    ...
    if(!result.ok){status.className='copilot-status error';status.innerHTML='本轮说明未完成，可重试分析；本操作不批准或提交案件。<button class="tbtn" onclick="analyzeCurrentCase(\'CASE_OPENED\')">重试分析</button>';return false;}
...
async function analyzeCurrentCase(trigger='CASE_OPENED'){await runAgentTurn('请基于当前案件版本，解释材料登记缺口、阻断原因、下一步和需要谁确认；明确尚未读取文件正文。',trigger,false);}
