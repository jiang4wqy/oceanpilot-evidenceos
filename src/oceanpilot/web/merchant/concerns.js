...
async function addConcern(){
  ...
  if(!data.original_source||!data.proposed_source){formError('concernError','请填写原来源及建议来源。');return;}
  if(data.original_source.length>120||data.proposed_source.length>120){formError('concernError','输入超过允许长度，请缩短后重试。');return;}
