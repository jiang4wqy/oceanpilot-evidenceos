  ...
  async function open(retryId = null) {
    ...
    try {
      ...
      fields.innerHTML = state.retry
        ...
        : `${field("event_type", "上游事件类型")}${field("source_event_id", "来源系统事件 ID")}${field("channel", "来源渠道")}${field("merchant_id", "对应商户 ID")}${field("transaction_id", "原交易 ID")}${field("scheme", "卡组织（VISA 或 MASTERCARD）")}${field("amount_minor", "交易金额（最小货币单位）")}${field("currency", "币种（例如 USD）")}${field("reason_code", "原始原因码")}${field("received_at", "实际收到事件的时间", "datetime-local")}${field("occurred_at", "上游事件发生时间", "datetime-local")}${field("case_template_id", "指南演练模板编号，例如 CB-CASE-041")}<p class="section-note">引用已核对的模板不会生成交易事实或自动确认规则。仍须先由IT 管理员登记对应合成交易。</p><details class="intake-extra"><summary>已有案件关联、撤回或更正依据</summary>${field("upstream_case_id", "来源系统案件编号")}${field("target_case_id", "明确关联的 OceanPilot 案件")}${field("stage_number", "事件所属轮次")}${field("corrects_event_id", "本次更正的原事件 ID")}${field("outcome", "上游明确的业务结果")}${field("final", "上游已明确确认终局")}${field("mapped_outcome", "其他结果的候选映射")}${field("supported_minor", "已确认支持金额（最小货币单位）")}${field("liable_minor", "已确认责任金额（最小货币单位）")}${field("reason", "结果与映射说明", "textarea")}${field("basis_reference", "核实与更正依据")}${field("authorization_reference", "授权依据")}<p class="section-note">部分支持或映射为部分支持时，须填写支持金额和责任金额；两者合计应与争议金额一致。未知和其他结果仍须由风控核验，填写映射不会自动确认终局。</p></details>`;
