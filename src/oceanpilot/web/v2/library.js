...
(() => {
  ...
  async function mount({ api, openTemplate, onCaseOpen } = {}) {
    ...
    function listMarkup() {
      ...
      return `<div class="library-list-label"><strong>${entries.length} 条参考</strong><span>${state.search || state.scheme !== "ALL" || state.level !== "ALL" ? "已按条件筛选" : "按原始条目编号排序"}</span></div>${entries.map((ref) => `<button type="button" class="library-reference ${state.selected === idOf(ref) ? "selected" : ""}" data-library-reference="${escape(idOf(ref))}" aria-pressed="${state.selected === idOf(ref)}"><div class="library-ref-top"><span class="library-ref-id">${escape(idOf(ref))}</span>${badge(levelOf(ref))}</div><h3>${escape(ref.title || ref.title_cn || idOf(ref))}</h3><p>${escape(ref.summary || ref.scenario_one_line || "查看原文依据与适用边界。")}</p><div class="library-ref-bottom"><span>${escape(ref.scheme || ref.source_scheme || "组织未注明")} · ${escape(ref.reason_code || "原因码未注明")}</span><span class="${catalogueAvailable(ref) ? "library-available" : "library-readonly"}">可创建独立演练 <b>→</b></span></div></button>`).join("") || '<div class="library-empty"><h3>没有符合条件的参考</h3><p>调整卡组织、资料性质或搜索内容。</p><button class="button secondary" data-library-clear>清除筛选</button></div>'}`;
    }
    ...
    async function click(event) {
      ...
      if (target.hasAttribute("data-library-create")) {
        ...
        if (!ref || typeof openTemplate !== "function" || state.templateBusy) return;
        ...
        try { await openTemplate(selected); }
          ...
          if (isCurrent() && selected === state.selected) { state.templateBusy = false; const button = host.querySelector("[data-library-create]"); if (button) { button.disabled = false; button.textContent = "用此案例模拟拒付"; } }
        }
      }
    }
